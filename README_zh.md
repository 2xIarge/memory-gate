...
| ~~旧~~ | **答案关键词 = oracle** | `keep <被标记的>` | 8/10 | ~~10/10~~ 已撤回，见第 2 条 |

诚实的头条是 C1，而它是个**零结果**：用一个没见过题目的分类器，闸门的成绩和不装闸门
一模一样。这不是噪声，是算术。盲集标记的那 4 条（`营收口径`、`数据环境`、`货币单位`、
`交付节奏`）**在对照组里本来就全活了**，而两条臂里真正丢掉的约束，无一例外都在它漏掉
那 6 条里面。在 `keep <被标记的>` 策略下，分诊覆盖度就是收益的硬上限——4/10 的覆盖度
换来 0/10 的收益。

**`keep all` 卡在预算上，不卡在钉住这件事上。** C2 钉了 58 条（549 token），而
`protect_budget_tokens` 解析出来是 200，所以真正每轮重发的只有最新的 23 条（198 token）。
它丢掉的那两条约束是**钉住了但从未被注入**——磁盘上受保护，模型一眼没看到。而且钉住的
token 里有 **72% 是闲聊**（`run query 7` 那一类），也就是说笼统全钉，把稀缺预算大部分
花在了不是规则的东西上。清单里确实把这话印出来了（"keep all 会钉 549 token，但预算只有
200……"），可这两个默认值彼此还没配上，而"给 pin 分优先级"这个显而易见的补法，本包
**没有**实现。
...
2. **这版 README 曾经拿来做头条的 8/10 → 10/10 已撤回。** 那一跑的 `protect_patterns`
   ...
   注入这条链路必须全部为真才出得来那个数，所以它不是没有价值，只是从来就不是分诊的证据。
3. **出厂默认集同样不是盲的。** 其中几个陈述句标记（`按这个来`、`习惯了`、`顺手记`、
   `只用`、`都这么`、`记得`）是我今天**照着这次实验的漏检清单**写进去的，所以 C2 和 R2
   的 10/10 覆盖度有一部分是自我实现的。`verify_real_model.py --patterns original` 就是
   为了把这个污染做成可复现的，而不是靠嘴解释掉；每次跑的头部会印出用的是哪套、与被植入
   原句的重叠、以及覆盖度。
